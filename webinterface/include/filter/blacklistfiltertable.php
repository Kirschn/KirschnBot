...
$sql = "SELECT id, word FROM wordblacklist WHERE channel='#".strtolower($username)."';";
    ...
    while ($r = mysqli_fetch_assoc($commandsunparsed)) {
        ...
        <tr>
            <td>
                ...
                <td>
                    <button type="button" class="btn btn-primary" onclick="$.get('function/removeblacklist.php?token=<?php echo $_SESSION['onetimetoken']; ?>&id=<?php echo $r["id"]; ?>').done(function() {$('#blacklist').load('include/filter/blacklistfiltertable.php')})">Remove</button>
                </td>
        </tr>
        <?php
    }
