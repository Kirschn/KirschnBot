...
client.addListener('message', function (from, to, message) {
    ...
    if (message.indexOf("!kdebottest") == 0 ) {
    	client.say(to, "Testmessage in channel " + to + " from " + from);
    }
    if (message.indexOf("!debug") == 0) {
    	if (ismod(from)) {
    		...
    		client.say(to, from + " -> This command is mod-only!");
    	}
    }
    if (message.indexOf("!level") == 0) {
    		...
    		client.say(to, splitmessage[1] + " Userlevel: " + String(getuserlevel(splitmessage[1], channel, from)));
    ...
    if (message.indexOf("!addcom") == 0) {
        ...
        if (getuserlevel(from, String(to.replace("#", "") >= 4))) {
            if (existscommand) {
                client.say(to, from + " -> This command exists already!");
            } else {
                ...
                client.say(to, from + " -> The command " + splitmessage1[0] + " was created successfully.");
            }
        }
    }
    if (message.indexOf("!removecom") == 0) {
        ...
        if (getuserlevel(from, String(to.replace("#", "") >= 4))) {
            ...
            if (existscommand) {
                client.say(to, from + " -> This command doesn't exist!");
            } else {
                fs.unlink("commands/!" + splitmessage3[1] + "/userlevel");
                fs.unlink("commands/!" + splitmessage3[1] + "/text");
                fs.rmdir("commands/!" + splitmessage3[1]);
                client.say(to, from + " -> The command " + splitmessage3[1] + " was deleted successfully.");
            }
        }
    }
    if (message.indexOf("!renamecom") == 0) {
        ...
        if (getuserlevel(from, String(to.replace("#", "") >= 4))) {
            if (existscommand) {
                client.say(to, from + " -> The command " + splitmessage3[1] + " doesn't exist!");
            } else {
                fs.rename("commands/!" + splitmessage3[1], "commands/!" + splitmessage3[2]);
                client.say(to, from + " -> The command " + splitmessage3[1] + " was successfully renamed into " + splitmessage3[2] + ".");
            }
        }
    }
    if (message.indexOf("!reinit") == 0) {
        if (getuserlevel(from, String(to.replace("#", "") >= 4))) {
    	client.say(to, "Reloading commandlist and userlist...");
    	fs.readdir("commands", function(err, files) {
    		commandlist = files;
    		if (!err) {
    		    readusers(to);
